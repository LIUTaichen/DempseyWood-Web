...
public class SummaryByLoadTypeSheetWriter   extends SheetWriter {

    public SummaryByLoadTypeSheetWriter(String sheetName){
    ...
    public void writeSheet(Workbook workbook, List<HaulSummary> summaryList){
        ...
        String[] columnNames = {"Load Type","Machine", "Load Count","Volume", "Time" ,"Cost", "Revenue" ,"Profit" };
    ...
    private List<HaulSummary> getSortedHaulSummaries(List<HaulSummary> summaryList) {
        return summaryList.stream().sorted(Comparator.comparing(HaulSummary::getLoadType).thenComparing(HaulSummary::getEquipment)).collect(Collectors.toList());
    }
}
