...
L
		.tileLayer(
				'https://api.tiles.mapbox.com/v4/{id}/{z}/{x}/{y}.png?access_token=pk.eyJ1IjoiamFzb25saXV0YWljaGVuIiwiYSI6ImNqNmZ5ZGpkcjAzYWIzNXA1aWs5OXF3bXcifQ.33nis-JWUXYo1jpJkr1OSQ',
				{
					maxZoom : 18,
...
$(function() {
	...
	drawButton.attr("data-content",
			"Click here to start defining loading and dumping zones");
...
/*
 * add a onlick function on each layer. this function opens the zone details
 * modal dialog if the map is not in a edit or delete session pulls details from
 * the layer object and populate the dialog form with them when OK button is
 * clicked, save the form input back to the layer object, as well as update the
 * database
 */
			...
			.on(
					...
					function() {
						if (isDetailDialogEnabled) {
							$('#material').val(
									layer.feature.properties.material);
							$('#zoneName').val(
									layer.feature.properties.zoneName);
							$(
									"input[name='zoneType'][value='"
											+ layer.feature.properties.zoneType
											+ "']").prop('checked', true);
							...
							$('#okButton')
									.on(
											...
											function() {
												console
														.log("closing modal, saving details ");
												layer.feature.properties.material = $(
														'#material').val();
												layer.feature.properties.zoneName = $(
														'#zoneName').val();
												layer
														.setTooltipContent(layer.feature.properties.zoneName);
												layer.feature.properties.zoneType = $(
														"input[name='zoneType']:checked")
														.val();
												console
														.log(layer.feature.properties);
...
function getObjectFromLayer(layer) {
	...
	for ( var k in layer.feature.properties)
		geofence[k] = layer.feature.properties[k];
	return geofence;
}
			...
			function(data) {
				...
				for (index in data) {
					...
					for (var i = 0; i < track.readings.length; i++) {
						coordinates[i] = [ track.readings[i].lat,
								track.readings[i].lng ];
					}
